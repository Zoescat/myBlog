...
# 创建连接池 连接池由全局变量 __pool存储，缺省情况下将编码设置为utf8，自动提交事务
...
# 执行select语句，用select 函数执行，需要传入SQL语句和SQL参数
...
# 定义一个execute()函数，执行insert、update、delete语句
...
class Model(dict, metaclass=ModelMetaclass):
    ...
    async def remove(self):
        ...
        if rows != 1:
            logging.warning('failed to remove primary key：affected rows: %s' % rows)
